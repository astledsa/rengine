...
_NOTE: The text index path **must** exist, but the vector index path **must not**, for example, only the `./storage/index` folder is needed, since coreNN on initialization creates the folder. While in case of tanvity, it does not, hence `./storage/index/text` must exist beforehand._
